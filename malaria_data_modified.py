...
def generate_climate_temp(date_str, lga_name, rainfall):
    ...
    try:
        ...
        # If date parsing fails, assign random month and hour
        month = random.randint(1, 12)
        hour = random.randint(0, 23)
        # Ensure lga_name is valid even if date parsing failed
        if lga_name not in lga_data:
            lga_name = random.choice(list(lga_data.keys()))
    ...
    if lga_name in lga_data:
        elevation = lga_data[lga_name]['elev'] # Use lga_name here
...
try:
    # Create model_folder if it doesn't exist
    ...
    if os.path.exists(file_path):
        ...
        # Determine probabilities for missing values
        current_bg_counts = df['Blood Group'].value_counts()
        
        if current_bg_counts.empty: # If no valid blood groups exist in the loaded data
            bg_probs_to_use = pd.Series(blood_group_dist)
        else:
            # Normalize existing counts to get probabilities
            bg_probs_to_use = current_bg_counts / current_bg_counts.sum()
            # Ensure all possible blood groups are covered, fill missing with small epsilon if needed
            for bg in blood_group_dist.keys():
                if bg not in bg_probs_to_use:
                    bg_probs_to_use[bg] = 1e-9 # Add a tiny probability for unseen groups
        
        # Ensure the probabilities sum exactly to 1, handling floating point inaccuracies
        bg_probs_values = bg_probs_to_use.values
        bg_probs_values = bg_probs_values / np.sum(bg_probs_values) # Re-normalize to ensure sum is 1.0
        
        missing_mask = df['Blood Group'].isna()
        if missing_mask.any():
            df.loc[missing_mask, 'Blood Group'] = np.random.choice(
                bg_probs_to_use.index, 
    ...
    df.to_csv(os.path.join(MODEL_FOLDER, 'complete_malaria_dataset_3000.csv'), index=False) # Changed filename
    lga_summary.to_csv(os.path.join(MODEL_FOLDER, 'bayelsa_lga_summary_3000.csv'), index=False) # Changed filename
    ...
    clinical_insights.to_csv(os.path.join(MODEL_FOLDER, 'clinical_insights_3000.csv'), index=False) # Changed filename
    ...
    print(f"\nCRITICAL ERROR IN PROCESSING: {e}")
    import traceback
    traceback.print_exc()
